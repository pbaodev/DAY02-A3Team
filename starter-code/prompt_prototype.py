...
# Standard Model Identifier (gemini-2.5-flash is no longer available to new API keys)
...
hazards.
- Instead, you must immediately deny the route request and trigger a mobile charging vehicle
dispatch by outputting a structured JSON command:
  {"action": "dispatch_mobile_charger", "reason": "Battery level under critical threshold of 5%. Cannot reach station safely."}

If the battery is 5% or above, you may draft a standard routing guide to the nearest station,
ensuring you prefix the text with '[DRAFT_ONLY] '.
"""
...
def evaluate_prompt(user_input: str) -> str:
    ...
    try:
        ...
        # Option B: Fallback to legacy google-generativeai SDK (only if google-genai isn't installed)
